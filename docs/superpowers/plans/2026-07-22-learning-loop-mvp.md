...
- [x] Write failing tests asserting the empty state schema, stable vocabulary IDs, Lesson-to-vocabulary mapping and configured review intervals.
- [x] Run `node --test tests/learning-domain.test.mjs` and verify expected missing-module failure.
- [x] Implement strict domain types, empty-state factory, configuration and a catalog derived from verified Lesson 1–2 source blocks.
...
- [x] Write failing tests with an in-memory Storage adapter for initialization, persistence, malformed JSON recovery, unsupported schema rejection, export/import replace, import merge and clear.
- [x] Run the repository test and observe failure because implementation is missing.
- [x] Implement storage-independent result/error contracts, migration registry and debounced writes with explicit `flush()` for critical operations.
- [x] Run repository and domain tests and verify PASS.
...
- [x] Write failing tests for start/update/complete Lesson, vocabulary scheduling, wrong-answer creation, two-correct mastery, repeated-wrong reset, task idempotency, daily statistics and next action.
- [x] Run the service test and verify the expected failure.
- [x] Implement pure schedulers and a LearningService that commits each atomic workflow through the Repository.
- [x] Run all learning tests and verify PASS.
...
- [x] Write a contract test asserting explicit hydration, Repository-only persistence and all required action names.
- [x] Run the contract test and verify failure.
- [x] Implement singleton local repository/service initialization on the client, explicit hydration and actions with unified error state.
- [x] Run TypeScript and learning tests.
...
- [x] Write UI contract tests for required labels, confirmation, last-position links and absence of direct localStorage calls.
- [x] Run tests and verify failure.
- [x] Implement Dashboard statistics/tasks and Lesson controls; update position from section interactions and complete the Lesson through one service action.
- [x] Run tests, TypeScript and ESLint.
...
- [x] Write UI contract tests for favorite/mastery, wrong/correct attempt controls, two-success remediation, JSON import/export and browser-only warning.
- [x] Run test and verify failure.
- [x] Implement the client pages without direct storage access.
- [x] Run all tests, TypeScript and ESLint.
...
- [x] Run `npm test`, `npx tsc --noEmit`, `npm run lint`, and `npm run build`.
...
- [x] Document the local Repository architecture and future Supabase merge paths.
