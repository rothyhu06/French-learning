...
export function AppShell({ children, active }: { children: React.ReactNode; active: string }) {
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="level-card"><div><span>教材索引</span><strong>4 课</strong></div><div className="mini-progress"><i style={{ width: "10%" }} /></div><small>已核对 Leçon 1–4</small></div>
    ...
    <main className="main"><header className="topbar"><div className="mobile-brand"><span className="brand-mark">f.</span><strong>Le Français</strong></div><div className="top-actions"><Link href="/search" aria-label="搜索教材"><Icon name="search" /></Link><span className="streak"><Icon name="book" /> 已核对 4 课</span><button className="avatar">学</button></div></header>{children}</main>
  </div>;
}
