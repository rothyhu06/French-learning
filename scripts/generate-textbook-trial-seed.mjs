...
for (const block of sourceBlocks) {
  ...
  if (block.lessonId) rows.push(`insert into public.content_source_links (source_block_id,entity_type,entity_id,relation_type,confidence,verification_status) values (${q(uuid(block.id))},'lesson',${q(uuid(block.lessonId))},'source',1,'verified') on conflict do nothing;`);
  if (block.sectionId) rows.push(`insert into public.content_source_links (source_block_id,entity_type,entity_id,relation_type,confidence,verification_status) values (${q(uuid(block.id))},'lesson_section',${q(uuid(block.sectionId))},'source',1,'verified') on conflict do nothing;`);
